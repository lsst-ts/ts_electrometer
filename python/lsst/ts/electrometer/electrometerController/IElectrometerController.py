...
class IElectrometerController(ABC):
    ...
    @abstractmethod
    def configureCommunicator(self, port, baudrate, parity, stopbits, bytesize,
                              byteToRead=1024, dsrdtr=False, xonxoff=False, timeout=2, termChar="\n"):
        ...
        Arguments:
            ...
            parity {long} -- Parity checking. PARITY_NONE=0, PARITY_EVEN=1,
                             PARITY_ODD=2, PARITY_MARK=3, PARITY_SPACE=4
    ...
    def verifyValidState(self, validStates, skipVerification=False):
        ...
        if(skipVerification):
            return
        if(self.getState() not in validStates):
            raise ValueError(f"{currentframe().f_code.co_name} not allowed in {self.getState().name} state")
