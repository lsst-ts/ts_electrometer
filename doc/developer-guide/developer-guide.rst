...
The CSC owns SAL-facing behavior such as events, detailed state transitions, image name allocation, and large file object publication.
The controller classes own device-facing behavior such as SCPI command sequencing, controller state, scan readout, and FITS file generation.
...
.. note::
    Flow-control settings must match on the electrometer and the Moxa.
    If an installation is operating over an unreliable wireless bridge, dropped TCP sessions or stale serial flow-control state can leave the device unresponsive until the connection or instrument is reset.
    In that case, test any change to XON/XOFF conservatively and apply the same flow-control choice on both ends of the serial link.
...
The package separates CSC responsibilities from controller responsibilities.

``ElectrometerCsc``
    Handles SAL commands and events, detailed states, S3 bucket setup, image name service interaction, simulator startup, and large file object publication.

``ElectrometerController``
    Handles common device workflows, including connection initialization, zero calibration, scan setup, scan readout, error queue checks, controller state caching, and in-memory FITS creation.

``KeithleyElectrometerController`` and ``KeysightElectrometerController``
    Provide the brand-specific command factories and behavior needed by the common controller workflow.

``Commander``
    Serializes TCP/IP command transactions and reconnects/retries when a command reply is interrupted by a connection drop.
    It reads replies in chunks until the device terminator is received, which avoids byte-at-a-time reads and handles longer replies such as scan buffers.

The controller does not publish SAL events directly.
After calling a controller method, the CSC publishes the relevant event using the updated value cached on the controller.
...
The command-line commander can be started with:

.. prompt:: bash

    command_electrometer 1
...
It is mostly meant for unit testing the CSC.
Simulation mode starts a mock TCP/IP server for the configured electrometer type and points the controller at that server.
...
The documentation uses Documenteer 2 with the ``documenteer[guide]`` extra.
The Sphinx preset is imported from ``doc/conf.py`` and project-specific
metadata, including intersphinx projects, is configured in
